...
#### Минимальная конфигурация (только CPU)

- **CPU**: 4+ ядра (Intel i5-8xxx / AMD Ryzen 5 3xxx или эквивалент)
- **RAM**: 12 ГБ
  - Ollama LLaMA 3.2 (3B): 4-6 ГБ
  - Stable Diffusion WebUI: 4-6 ГБ
  - Система + Сервисы: 2-4 ГБ
- **GPU**: Не требуется (поддерживается инференс на CPU)

**Ожидаемая производительность:** 5-10 минут на генерацию мема

---

#### Рекомендуемая конфигурация (с ускорением GPU)

- **CPU**: 6+ ядер (Intel i7-10xxx / AMD Ryzen 7 5xxx или лучше)
- **RAM**: 16 ГБ
- **GPU**: NVIDIA GPU с 6+ ГБ VRAM
  - **Минимум**: GTX 1660 Ti (6 ГБ VRAM)
  - **Рекомендуется**: RTX 3060 (12 ГБ VRAM)
  - **Оптимально**: RTX 4070/4080 (12-16 ГБ VRAM)

**Ожидаемая производительность:** 15-60 секунд на генерацию мема

---

#### Требования Docker

- **RAM**: минимум 16 ГБ (рекомендуется 24 ГБ для мульти-сервисной установки)
- **CPU**: Как указано выше
- **GPU**: Опционально, требуется NVIDIA Docker runtime + поддержка CUDA
...
- Docker Engine 20.10+
- Docker Compose 2.0+
- Минимум 8 ГБ RAM (рекомендуется 16 ГБ для ML моделей)
...
#### 2️⃣ Настройте файл .env по примеру .env.example
...
#### 3️⃣ Установите Stable Diffusion WebUI

##### Установка WebUI

👉 https://github.com/AUTOMATIC1111/stable-diffusion-webui

##### Запустите WebUI с включенным API
...
#### 4️⃣ Запустите Docker контейнеры

```bash
docker compose up -d
```

\*Убедитесь, что Stable Diffusion запущен локально или в Docker с [stable-diffusion-webui-docker-master](stable-diffusion-webui-docker-master/docker-compose.yml)
...
| ---------------- | ----- | ---------------------------- | ------------------------------- |
| API Service      | 8000  | http://localhost:8000        | Основной сервис API             |
...
| Ollama Service   | 11434 | http://localhost:11434       | Сервис LLM модели               |
| Stable Diffusion | 7860  | http://localhost:7860        | WebUI для генерации изображений |

#### Теперь вы можете использовать его через [Swagger](http://localhost:8000/docs) или curl.
...
#### 2️⃣ Создайте виртуальное окружение Python
...
\*Вы можете использовать другую модель
...
Ollama будет работать по адресу: `http://localhost:11434`
...
##### 2️⃣ Запустите WebUI с включенным API
...
#### 3️⃣ (Опционально) Добавьте пользовательские модели

Загрузите дополнительные модели с [Civitai](https://civitai.com/):

| Модель           | Стиль                           | Ссылка                                                     |
| ---------------- | ------------------------------- | ---------------------------------------------------------- |
| **Memes XL**     | Мемный стиль, смешной, яркий    | [Скачать](https://civitai.com/models/205229/memes-xl)      |
| **Crazy Horror** | Темный, сюрреалистичный, хоррор | [Скачать](https://civitai.com/models/1101129/crazy-horror) |

Поместите скачанные файлы `.safetensors` в:

```
stable-diffusion-webui/models/Stable-diffusion/
```

#### 4️⃣ Запустить демо-код приложения
...
| ------------------------ | ----------------------------- | ------------------- |
| `OLLAMA_MODEL`           | Название LLM модели           | `alibayram/smollm3` |
...
Основные эндпоинты API для генерации мемов:

- _/api/memes/generate-template_ - создание среднестатистических мемов с помощью [шаблонов](https://github.com/jacebrowning/memegen)
- _/api/memes/generate_ - создание мемов с помощью Stable Diffusion
- _/api/memes/styles_ - список доступных стилей мемов
- _/api/memes/task/{task_id}_ - получение статуса генерации мема под номером task*id, который выдаётся после запроса на */api/memes/generate\_
- _/api/memes/task/{task_id}/result_ - получение готового мема
